"""Add promotions tables and fix tier_configurations
...
def upgrade():
    ...
    if not table_exists(conn, 'promotions'):
    ...
    # Create store_credit_ledger table if it doesn't exist
    if not table_exists(conn, 'store_credit_ledger'):
    ...
    # Create member_credit_balances table if it doesn't exist
    if not table_exists(conn, 'member_credit_balances'):
    ...
    # Create bulk_credit_operations table if it doesn't exist
    if not table_exists(conn, 'bulk_credit_operations'):
    ...
    if not table_exists(conn, 'tier_configurations'):
        op.create_table('tier_configurations',
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('tier_name', sa.String(length=20), nullable=False),
            sa.Column('monthly_price', sa.Numeric(precision=6, scale=2), nullable=False),
            ...
            sa.Column('color', sa.String(length=20), nullable=True, server_default='slate'),
        ...
        if not column_exists(conn, 'tier_configurations', 'icon'):
